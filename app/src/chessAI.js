class ChessAI {
  ...
  constructor(game){
    this.game = game;
    this.globalSum = 0; // always from black's perspective. Negative for white's perspective.
    this.positionCount;
    this.searchDepth = 3;
    this.searchDepthWhite  = 3;
    /*
   ...
   evaluateBoard(game, move, prevSum, color) {
      ...
      let pstOpponent = this.pstOpponent;
      let pstSelf = this.pstSelf;
    ...
    getBestMove(game, color, currSum) {
      ...
      console.log('#positions-per-s = ' + Math.round(positionsPerS));
    ...
    makeBestMove(color) {
     let game = this.game;
     let globalSum = this.globalSum;

      if (color === 'b') {
        var move = this.getBestMove(game, color, globalSum)[0];
      } else {
        var move = this.getBestMove(game, color, -globalSum)[0];
      }

      globalSum = this.evaluateBoard(game, move, globalSum, 'b');
